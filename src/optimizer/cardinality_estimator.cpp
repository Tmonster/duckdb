...
//	auto left_binding = filter_info->left_binding;
//	auto right_binding = filter_info->right_binding;
...
//	auto right_binding = filter_info->right_binding;
	...
	if (matching_equivalent_sets.size() > 1) {
		...
		unordered_set<ColumnBinding> tmp = {filter_info->left_binding, filter_info->right_binding};
		equivalent_relations.push_back(tmp);
	}
}
...
void CardinalityEstimator::InitTotalDomains() {
	vector<unordered_set<ColumnBinding>>::iterator it;
			...
			for (auto &child_filter : fil.child_filters) {
				if (child_filter->filter_type == TableFilterType::CONSTANT_COMPARISON) {
					...
					if (comparison_filter.comparison_type == ExpressionType::COMPARE_EQUAL) {
						auto base_stats = catalog_table->storage->GetStatistics(context, it->first);
